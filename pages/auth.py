...
import sys, os, base64, pathlib, secrets
...
[data-testid="stHorizontalBlock"] {
    ...
    min-height: 100vh; width: 100% !important; flex-wrap: nowrap !important;
}
[data-testid="stHorizontalBlock"] > div { padding: 0 !important; min-height: 100vh; flex-shrink: 0 !important; }
[data-testid="stHorizontalBlock"] > div:first-child { display: flex; flex-direction: column; }
[data-testid="stHorizontalBlock"] > div:last-child {
    background: #eef2f9 !important;
    display: flex !important; flex-direction: column !important;
    align-items: center !important; justify-content: flex-start !important;
    padding: 24px 20px !important; overflow-y: auto !important;
    min-height: 100vh; min-width: 340px !important;
}
...
[data-testid="stVerticalBlockBorderWrapper"] > div { padding: 24px 24px 28px !important; border: none !important; }
.left-panel {
    flex: 1;
    ...
    border-radius: 0 28px 28px 0; min-height: 100vh;
}
.left-panel .grid-bg {
...
.left-logo { width: 250px; margin-left: -280px; margin-bottom: 48px; filter: drop-shadow(0 4px 18px rgba(0,0,0,.35)); z-index: 1; position: relative; }
.left-mockup { position: relative; width: 100%; max-width: 550px; z-index: 1; margin-bottom: 36px; }
.left-mockup .img-main { width: 100%; border-radius: 16px; box-shadow: 0 24px 64px rgba(0,0,0,.5); display: block; }
...
.left-tagline h2 { font-size: 36px; font-weight: 700; letter-spacing: .6px; line-height: 1.4; margin-bottom: 14px; text-transform: uppercase; }
.left-tagline p { font-size: 20px; opacity: .80; line-height: 1.7; max-width: 450px; margin: 0 auto; }
[data-testid="stTabs"] { margin: 0 !important; }
[data-testid="stTabs"] [role="tablist"] {
    border-bottom: 2px solid #e4eaf5 !important; gap: 0 !important;
    background: transparent !important; padding: 0 !important; margin: 0 0 4px 0 !important;
}
[data-testid="stTabs"] button[role="tab"] {
    font-family: 'Be Vietnam Pro', sans-serif !important; font-weight: 600 !important;
    font-size: .97rem !important; color: #9baabf !important; padding: 10px 28px !important;
...
[data-testid="stTabs"] button[role="tab"][aria-selected="true"] { color: #1557d4 !important; border-bottom: 3px solid #1557d4 !important; }
...
.form-heading { font-size: 1.45rem; font-weight: 700; color: #1557d4; margin-bottom: 20px; text-align: center; font-family: 'Be Vietnam Pro', sans-serif; }
.field-label { font-size: .82rem; font-weight: 600; color: #3a4a6a; margin-bottom: 4px; margin-top: 12px; font-family: 'Be Vietnam Pro', sans-serif; line-height: 1; }
.forgot-row { display: flex; justify-content: flex-end; margin-top: 4px; }
.forgot-row a { font-size: .8rem; color: #1557d4; text-decoration: none; font-weight: 500; }
[data-testid="stTextInput"] label, [data-testid="stNumberInput"] label { display: none !important; }
[data-testid="stTextInput"] > div > div {
    height: 40px !important; min-height: 40px !important;
    display: flex !important; align-items: center !important;
    border: 1.5px solid #d0d8e8 !important; border-radius: 7px !important;
    background: #fff !important; box-shadow: none !important;
    overflow: hidden !important; transition: border-color .2s !important;
}
...
[data-testid="stTextInput"] > div > div input {
    height: 40px !important; padding: 0 16px !important; border: none !important;
    background: transparent !important; font-size: .95rem !important;
    color: #1c2e4a !important; font-family: "Be Vietnam Pro", sans-serif !important;
    flex: 1 !important;
}
[data-testid="stTextInput"] > div > div input::placeholder { color: #b0bccc !important; }
[data-testid="stButton"] { width: 100% !important; display: block !important; }
[data-testid="stButton"] > button {
    width: 100% !important; display: block !important;
    background: #1557d4 !important; color: #fff !important;
    border: none !important; border-radius: 10px !important; height: 52px !important;
    font-family: 'Be Vietnam Pro', sans-serif !important; font-size: 1rem !important;
    font-weight: 700 !important; cursor: pointer !important; margin-top: 20px !important;
    box-shadow: 0 4px 18px rgba(21,87,212,.28) !important;
}
[data-testid="stButton"] > button:hover { background: #0d40b8 !important; transform: translateY(-1px) !important; }
[data-testid="stButton"] > button p { font-size: 1rem !important; font-weight: 700 !important; color: #fff !important; }
[data-testid="stAlert"] { border-radius: 10px !important; font-family: 'Be Vietnam Pro', sans-serif !important; font-size: .88rem !important; margin-top: 10px !important; }
</style>
...
with left_col:
    logo_html = f'<img class="left-logo" src="{LOGO}" alt="Logo">' if LOGO else '<div style="color:#fff;font-size:1.8rem;font-weight:800;margin-bottom:28px;z-index:1;position:relative;">SEA DRAGON</div>'
    img_main_html = f'<img class="img-main" src="{ANH1}" alt="Dashboard">' if ANH1 else '<div style="background:rgba(255,255,255,.13);height:220px;border-radius:16px;"></div>'
...
with right_col:
    with st.container():
        ...
        with tab_login:
            ...
            username = st.text_input("login_user", placeholder="Tên tài khoản", label_visibility="collapsed")
            st.markdown('<span class="field-label">Mật khẩu</span>', unsafe_allow_html=True)
            password = st.text_input("login_pass", placeholder="••••••••", type="password", label_visibility="collapsed")
            st.markdown('<div class="forgot-row"><a href="#">Quên mật khẩu ?</a></div>', unsafe_allow_html=True)
        ...
        with tab_register:
            ...
            new_display_name = st.text_input("reg_dn", placeholder="Nguyễn Văn A", label_visibility="collapsed", key="register_display_name")
            st.markdown('<span class="field-label">Tên đăng nhập</span>', unsafe_allow_html=True)
            new_username = st.text_input("reg_u", placeholder="username", label_visibility="collapsed", key="register_username")
            st.markdown('<span class="field-label">Mật khẩu</span>', unsafe_allow_html=True)
            new_password = st.text_input("reg_p", placeholder="••••••••", type="password", label_visibility="collapsed", key="register_password")
            st.markdown('<span class="field-label">Xác nhận mật khẩu</span>', unsafe_allow_html=True)
            confirm_password = st.text_input("reg_cp", placeholder="••••••••", type="password", label_visibility="collapsed", key="register_confirm")
            st.markdown('<span class="field-label">Số domain</span>', unsafe_allow_html=True)
            num_domains = st.number_input("reg_nd", min_value=1, max_value=10, value=1, label_visibility="collapsed")
            ...
            for i in range(int(num_domains)):
                with st.expander(f"Domain {i + 1}"):
                    ...
                    worksheet = st.text_input("Tên worksheet",  key=f"ws_{i}",     value="Ngày_22_01_2026")
                ...
                if new_password != confirm_password:
                    ...
                    if success:
                        ...
                        if succ:
                            ...
                            st.success(f"✅ {msg} — Bạn có thể đăng nhập ngay")
                    else:
                        st.error(f"❌ {msg}")

# ── Logout button (hiển thị nếu đang đăng nhập) ──────────────────────────────
if is_authenticated():
